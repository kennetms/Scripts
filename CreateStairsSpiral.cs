...
public class CreateStairsSpiral : MonoBehaviour {
	...
	IEnumerator CreateStairsRoutine(){
		...
		while (StairsSetup.amountOfStairs > stairNr){
			...
			if (StairsSetup.stairMaterial != null){
				...
				stairs.GetComponent<Renderer>().material = StairsSetup.stairMaterial;
			}
			
			if (StairsSetup.stairsRigidbody){
				
				stairs.AddComponent<Rigidbody>();
				stairs.GetComponent<Rigidbody>().angularDrag = 0f;
				...
				if (StairsSetup.stairsGravity){
                    ...
                    stairs.GetComponent<Rigidbody>().useGravity = false;
				}
			...
			if (PanelSetup.createPanel){
				...
				if (PanelSetup.createPanel && PanelSetup.stairPanelMaterial != null){
					...
					stairs.GetComponent<Renderer>().material = PanelSetup.stairPanelMaterial;
				}
			...
			if (RailingSetup.createRailing){
				...
				if (RailingSetup.railingMaterial != null){
					...
					railing.GetComponent<Renderer>().material = RailingSetup.railingMaterial;
				}
				...
				if (RailingSetup.railingRigidbody){
					...
					railing.GetComponent<Rigidbody>().angularDrag = 0f;

					if (RailingSetup.railingGravity){

						railing.GetComponent<Rigidbody>().useGravity = true;

					} else if (!RailingSetup.railingGravity){

						railing.GetComponent<Rigidbody>().useGravity = false;
					}

					if (RailingSetup.railingKinematic){

						railing.GetComponent<Rigidbody>().isKinematic = true;
					
					} else if (!RailingSetup.railingKinematic){

						railing.GetComponent<Rigidbody>().isKinematic = false;
					}
				...
				if (stairNr == 0){
					...
					if (RailingSetup.railHandleMaterial != null){
						...
						railingHandle.GetComponent<Renderer>().material = RailingSetup.railHandleMaterial;
					}

					if (RailingSetup.handleRigidbody){
						
						railingHandle.AddComponent<Rigidbody>();
						railingHandle.GetComponent<Rigidbody>().angularDrag = 0f;

						if (RailingSetup.handleGravity){
							
							railingHandle.GetComponent<Rigidbody>().useGravity = true;
							
						} else if (!RailingSetup.handleGravity){
							
							railingHandle.GetComponent<Rigidbody>().useGravity = false;
						}
						
						if (RailingSetup.handleKinematic){
							
							railingHandle.GetComponent<Rigidbody>().isKinematic = true;
							
						} else if (!RailingSetup.handleKinematic){
							
							railingHandle.GetComponent<Rigidbody>().isKinematic = false;
						}
					}
			...
			if (StairsSetup.angledStairCollider){
				...
				if (StairsSetup.stairSize.y + StairsSetup.VerticalRangeBetween >= 0){
					...
					if (stairNr < amountOfColliders){
						...
						stairMoveCollidersStart.GetComponent<Renderer>().enabled = false;
						stairMoveCollidersEnd.GetComponent<Renderer>().enabled = false;
	...
	void CreateFrontPanelMesh (GameObject frontPanelMesh){
		...
		frontPanelMesh.GetComponent<Renderer>().material = PanelSetup.panelFrontMeshMaterial;
